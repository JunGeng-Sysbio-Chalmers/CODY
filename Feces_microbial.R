Feces_microbial <- function(data,Regime,legend_label) {
# data<-data_feces_microbial_all
...
p <-ggplot(data, aes(x=Regime,y=value,fill=Species))
# p<-p + theme_minimal()
p<-p+theme(panel.grid=element_blank(),axis.line=element_line(size=0.1,colour="#5e5e5e"),
...
p<-p+theme(
  ...
  # panel.border = element_blank(fill="#efefef"),
...
p <- p + labs(
  ...
  x="Diet Regime",y="Microbial level",
...
# if(legend_label==1)
  p<- p + theme(legend.position="bottom",legend.text=element_text(size=22),legend.title=element_blank())+guides(fill=guide_legend(ncol=2,byrow=FALSE))
