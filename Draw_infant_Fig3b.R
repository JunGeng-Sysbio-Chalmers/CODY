Draw_infant_Fig3b <- function(exp_data,prediction_data) {
  # prediction_data<-read.table(file="Preidiction_RA_exp_infant.csv", sep=";", header=T, stringsAsFactors=F )
####################
# load data
####################
...
labFUN <- function(x) {
  ...
  else if(x<0.1) return(round(x,2))
  else if(x>0.1) return(round(x,1))
}

max_value5<-max(main5_prediction[,3])
max_lab5<-labFUN(max_value5*1.6)

max_value3<-max(other3_prediction[,3])
max_lab3<-labFUN(max_value3*1.6)
...
main5_predict_plot <- ggplot(main5_prediction, aes(y=prediction, x=Type, colour=Species, fill=Species, shape=Type)) + 
  ...
  scale_y_continuous(expand=c(0.003,0), breaks = c(0,max_lab5/3,max_lab5*2/3,max_lab5), limits=c(0, max_lab5)) +
  ...
  theme(panel.grid.major = element_blank(), 
        ...
        plot.margin = unit(c(-1, 0.15, 0, 0.1), "pt"),
...
other3_predict_plot <- ggplot(other3_prediction, aes(y=prediction, x=Type, colour=Species,fill=Species, shape=Type)) + 
  ...
  theme(panel.grid.major = element_blank(), 
        ...
        # plot.margin = unit(c(-1, 0, 0, 0.1), "pt")
        plot.margin = unit(c(-1, 0, 0, 2), "pt") ## top, right, bottom, left
...
multi_plot <- ggarrange(main5_predict_plot, 
                        other3_predict_plot+rremove("ylab"),
                        nrow=1,
                        ncol=2,
                        widths=c(5,3.1)
