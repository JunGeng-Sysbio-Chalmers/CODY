Box_adult_Fig3e <- function(exp_file,prediction_file) {
...
main5_predict_plot <- ggplot(main5_prediction, aes(y=prediction*0.01, x=Time, fill=strain, shape=Time)) + 
  xlab("") + 
  ylab("Prediction") +
...
other3_predict_plot <- ggplot(other3_prediction, aes(y=prediction*0.01, x=Time, fill=strain, shape=Time)) + 
  xlab("") + 
  ylab("Prediction RA") +
...
ggsave(plot=multi_plot, filename="multi_plot-0726.pdf", width=18, height=11, units="cm")
